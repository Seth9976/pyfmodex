...
---
	...
	function m.androidCategoryLink(cfg, file)
		...
		if external then
			m.element(cfg, "Link", nil, path.translate(link))
		end
	end
						...
						for cfg in project.eachconfig(prj) do
							...
							if fcfg and fcfg[fld.name] then
								...
								if value and #value > 0 then
									m.element(fcfg, prop.name, m.configPair(cfg), '%s', value)
								end
							end
						end
					end
...
--	AUDIOKINETIC end
	...
	function m.languageStandard(cfg, condition)
		if _ACTION >= "vs2017" then
			if (cfg.cppdialect == "C++14") then
				m.element(cfg, "LanguageStandard", condition, 'stdcpp14')
			elseif (cfg.cppdialect == "C++17") then
				m.element(cfg, "LanguageStandard", condition, 'stdcpp17')
			elseif (cfg.cppdialect == "C++20") then
				m.element(cfg, "LanguageStandard", condition, iif(_ACTION <= "vs2017", 'stdcpplatest', 'stdcpp20'))
			elseif (cfg.cppdialect == "C++23") then
				m.element(cfg, "LanguageStandard", condition, 'stdcpplatest')
	...
	function m.languageStandardC(cfg, condition)
		if _ACTION >= "vs2019" then
			if (cfg.cdialect == "C11") then
				m.element(cfg, "LanguageStandard_C", condition, 'stdc11')
			elseif (cfg.cdialect == "C17") then
				m.element(cfg, "LanguageStandard_C", condition, 'stdc17')
			elseif (cfg.cdialect == "C23") then
				m.element(cfg, "LanguageStandard_C", condition, 'stdclatest')
			end
		end
	end
	...
	function m.compileAsWinRT(cfg, condition)
		if _ACTION >= "vs2019" then
			if cfg and cfg.consumewinrtextension ~= nil then
				m.element(cfg, "CompileAsWinRT", condition, iif(cfg.consumewinrtextension, "true", "false"))
			end
		end
	end
	...
	function m.additionalLinkOptions(cfg)
		...
		if #opts > 0 then
			...
			m.element(cfg, "AdditionalOptions", condition, '%s %%(AdditionalOptions)', additionalOptions)
		end
	end
	...
	function m.appContainerApplication(prj)
		if prj.system == p.UWP then
			m.element(prj, "AppContainerApplication", nil, "true")
		end
	end
	...
	function m.wholeProgramOptimization(cfg)
		...
		elseif cfg.linktimeoptimization == "Off" then
			m.element(cfg, "WholeProgramOptimization", nil, "false")
		end
	end
	...
	function m.destinationFolders(filecfg, condition)
		if filecfg then
			m.element(cfg, "DestinationFolders", condition, vstudio.path(filecfg.config, filecfg.config.buildtarget.directory))
		end
	end
	...
	function m.profile(cfg)
		if cfg.profile ~= nil then
			m.element(cfg, "Profile", nil, iif(cfg.profile, "true", "false"))
		end
	end
	...
	function m.ignoreImportLibrary(cfg)
		if cfg.kind == p.SHAREDLIB then
			...
			elseif cfg.system == p.UWP then
				m.element(cfg, "IgnoreImportLibrary", nil, "false")
			end
		end
	end
	...
	function m.linkTimeCodeGeneration(cfg)
		if cfg.linktimeoptimization == "On" then
			m.element(cfg, "LinkTimeCodeGeneration", nil, "UseLinkTimeCodeGeneration")
		end
	end
	...
	function m.llvmTools(cfg)
		...
		if llvmdir and _ACTION >= "vs2019" then
			m.element(cfg, "LLVMInstallDir", nil, vstudio.path(cfg, llvmdir))
		end

		if llvmversion and _ACTION >= "vs2019" then
			m.element(cfg, "LLVMToolsVersion", nil, llvmversion)
		end
	end

	function m.enableUnityBuild(cfg)
		if _ACTION >= "vs2017" and cfg.enableunitybuild then
			m.element(cfg, "EnableUnitySupport", nil, iif(cfg.enableunitybuild == "On", "true", "false"))
		end
	end
	...
	function m.precompiledHeaderFile(fileName, cfg)
		m.element(cfg, "PrecompiledHeaderFile", nil, "%s", fileName)
...
--		end -- AUDIOKINETIC
	...
	function m.clangtidy(cfg)
		if _ACTION >= "vs2019" and cfg.clangtidy ~= nil then
			m.element(cfg, "EnableClangTidyCodeAnalysis", nil, iif(cfg.clangtidy, "true", "false"))
		end
	end

	function m.runcodeanalysis(cfg)
		if _ACTION >= "vs2019" and cfg.runcodeanalysis ~= nil then
			m.element(cfg, "RunCodeAnalysis", nil, iif(cfg.runcodeanalysis, "true", "false"))
		end
	end
	...
	function m.windowsTargetPlatformVersion(prj, cfg)
		...
		if maxversion and target.system == p.UWP then
			m.element(prj, "WindowsTargetPlatformMinVersion", nil, minversion)
			m.element(prj, "WindowsTargetPlatformVersion", nil, maxversion)
		elseif minversion then
			m.element(prj, "WindowsTargetPlatformVersion", nil, minversion)
		end
	end
		...
		local function recurseTableIfNeeded(tbl)
			for key, value in spairs(tbl) do
				if (type(value) == "table") then
					...
					m.element(cfg, key, nil, vs2010.esc(value))
				end
			end
		end
	...
	function m.treatWarningAsError(cfg)
		if p.hasFatalCompileWarnings(cfg.fatalwarnings) and cfg.warnings ~= p.OFF then
			-- AUDIOKINETIC BEGIN: PS5 uses a different attribute for this
			if cfg.system == 'ps5' then
				m.element(cfg, "WarningsAsErrors", nil, "true")
			else
	...
	function m.useOfMfc(cfg)
		...
		elseif (cfg.mfc == "Off") then
			m.element(cfg, "UseOfMfc", nil, "false")
		elseif (cfg.mfc == "Static") then
			m.element(cfg, "UseOfMfc", nil, "Static")
		elseif (cfg.mfc == "Dynamic") then
			m.element(cfg, "UseOfMfc", nil, "Dynamic")
		end
	end
	...
	function m.enableModules(cfg)
		if _ACTION >= "vs2019" then
			if cfg.enablemodules then
				m.element(cfg, "EnableModules", nil, iif(cfg.enablemodules == "On", "true", "false"))
			end
		end
	end
	...
	function m.buildStlModules(cfg)
		if _ACTION >= "vs2022" then
			if cfg.buildstlmodules then
				m.element(cfg, "BuildStlModules", nil, iif(cfg.buildstlmodules == "On", "true", "false"))
			end
		end
	end
	...
	function m.useStandardPreprocessor(cfg)
		if _ACTION >= "vs2019" and cfg.usestandardpreprocessor ~= nil then
			if cfg.usestandardpreprocessor == 'On' then
				m.element(cfg, "UseStandardPreprocessor", nil, "true")
			else
				m.element(cfg, "UseStandardPreprocessor", nil, "false")
			end
		end
	end
	...
	function m.fxCompileAdditionalIncludeDirs(cfg, condition)
		if cfg.shaderincludedirs and #cfg.shaderincludedirs > 0 then
			...
			m.element(cfg, 'AdditionalIncludeDirectories', condition, "%s;%%(AdditionalIncludeDirectories)", table.concat(dirs, ";"))
		end
	end
...
--
	...
	function m.linuxApplicationType(cfg)
		m.element(cfg, "Keyword", nil, "Linux")
		m.element(cfg, "RootNamespace", nil, "%s", cfg.project.name)
		m.element(cfg, "MinimumVisualStudioVersion", nil, "15.0")
		m.element(cfg, "ApplicationType", nil, "Linux")
		m.element(cfg, "TargetLinuxPlatform", nil, "Generic")
		m.element(cfg, "ApplicationTypeRevision", nil, "1.0")
	end
	...
	function m.linuxDebugInformationFormat(cfg)
		if cfg.symbols ~= "Default" then
			...
			if cfg.symbols == p.OFF then
				m.element(cfg, "DebugInformationFormat", nil, "None")
			elseif cfg.symbols == "Full" then
				m.element(cfg, "DebugInformationFormat", nil, "FullDebug")
			else
				m.element(cfg, "DebugInformationFormat", nil, "Minimal")
			end
	...
	function m.linuxExceptionHandling(cfg, condition)
		if cfg.exceptionhandling then
			m.element(cfg, "ExceptionHandling", condition, iif(cfg.exceptionhandling == p.OFF, "Disabled", "Enabled"))
		end
	end
	...
	function m.linuxFloatingPointModel(cfg)
		...
		if cfg.floatingpoint then
			m.element(cfg, "RelaxIEEE", nil, iif(cfg.floatingpoint == p.OFF, "false", "true"))
		end
	...
	function m.linuxLanguageStandardCpp(cfg)
		...
		if cpp_langmap[cfg.cppdialect] ~= nil then
			m.element(cfg, "CppLanguageStandard", nil, cpp_langmap[cfg.cppdialect])
		end
	end
	...
	function m.linuxLanguageStandardC(cfg)
		...
		if c_langmap[cfg.cdialect] ~= nil then
			m.element(cfg, "CLanguageStandard", nil, c_langmap[cfg.cdialect])
		end
	end

	function m.remoteDeployDir(cfg)
		if cfg.remotedeploydir ~= nil then
			m.element(cfg, "RemoteDeployDir", nil, cfg.remotedeploydir)
		end
	end

	function m.remoteProjectRelDir(cfg)
		if cfg.remoteprojectrelativedir ~= nil then
			m.element(cfg, "RemoteProjectRelDir", nil, cfg.remoteprojectrelativedir)
		end
	end

	function m.remoteProjectDir(cfg)
		if cfg.remoteprojectdir ~= nil then
			m.element(cfg, "RemoteProjectDir", nil, cfg.remoteprojectdir)
		end
	end

	function m.remoteRootDir(cfg)
		if cfg.remoterootdir ~= nil and cfg.remoterootdir ~= "" then
			m.element(cfg, "RemoteRootDir", nil, cfg.remoterootdir)
		end
	end

	function m.linuxPIC(cfg, condition)
		if cfg.pic ~= nil then
			m.element(cfg, "PositionIndependentCode", condition, iif(cfg.pic == "On", "true", "false"))
		end
	end
	...
	function m.linuxStlType(cfg)
		if cfg.staticruntime ~= nil then
			m.element(cfg, "UseOfStl", nil, iif(cfg.staticruntime == "On", "libstdc++_static", "libstdc++_shared"))
		end
	end

	function m.linuxStrictAliasing(cfg)
		if cfg.strictaliasing ~= nil then
			m.element(cfg, "StrictAliasing", nil, iif(cfg.strictaliasing == "Off", "false", "true"))
		end
	end

	function m.linuxWarningLevel(cfg, condition)
		if cfg.warnings then
			m.element(cfg, "WarningLevel", condition, iif(cfg.warnings == p.OFF, "TurnOffAllWarnings", "EnableAllWarnings"))
		end
	end

	function m.linuxWholeProgramOptimization(cfg)
		if cfg.linktimeoptimization == "On" then
			m.element(cfg, "LinkTimeOptimization", nil, "true")
		elseif cfg.linktimeoptimization == "Off" then
			m.element(cfg, "LinkTimeOptimization", nil, "false")
		end
	end
	...
	function m.linuxMultiProcNumber(cfg)
		...
		if cfg.flags.MultiProcessorCompile then
			m.element(cfg, "MultiProcNumber", nil, "8")
		end
	end
...
--
	...
	function m.androidApplicationType(cfg)
		m.element(cfg, "Keyword", nil, "Android")
		m.element(cfg, "RootNamespace", nil, "%s", cfg.project.name)
		if _ACTION >= "vs2019" then
			m.element(cfg, "MinimumVisualStudioVersion", nil, "16.0")
		elseif _ACTION >= "vs2017" then
			m.element(cfg, "MinimumVisualStudioVersion", nil, "15.0")
		elseif _ACTION >= "vs2015" then
			m.element(cfg, "MinimumVisualStudioVersion", nil, "14.0")
		end
		m.element(cfg, "ApplicationType", nil, "Android")
		if _ACTION >= "vs2017" then
			m.element(cfg, "ApplicationTypeRevision", nil, "3.0")
		elseif _ACTION >= "vs2015" then
			m.element(cfg, "ApplicationTypeRevision", nil, "2.0")
		else
			m.element(cfg, "ApplicationTypeRevision", nil, "1.0")
		end
	end
	...
	function m.androidDebugInformationFormat(cfg)
		if cfg.symbols ~= "Default" then
			...
			elseif cfg.symbols == "Full" then
				...
				m.element(cfg, "DebugInformationFormat", nil, "LineNumber")
			end
	...
	function m.androidExceptionHandling(cfg, condition)
		...
		if _ACTION >= "vs2015" then
			if exceptions[cfg.exceptionhandling] ~= nil then
				m.element(cfg, "ExceptionHandling", condition, exceptions[cfg.exceptionhandling])
			end
		else
			if cfg.exceptionhandling == premake.ON then
				m.element(cfg, "GccExceptionHandling", condition, "true")
			end
		end
	end
	...
	function m.androidWarningLevel(cfg, condition)
		...
		if _ACTION >= "vs2015" then
			if cfg.warnings and cfg.warnings ~= "Off" then
				m.element(cfg, "WarningLevel", nil, "EnableAllWarnings")
			else
...
--				end
	...
	function m.androidEnableEnhancedInstructionSet(cfg)
		if cfg.vectorextensions == "NEON" then
			m.element(cfg, "EnableNeonCodegen", nil, "true")
		end
	end

	function m.androidPrecompiledHeaderFile(fileName, cfg)
		-- Doesn't work for project-relative paths.
		m.element(cfg, "PrecompiledHeaderFile", nil, "%s", path.getabsolute(path.rebase(fileName, cfg.basedir, cfg.location)))
	end

	function m.androidPIC(cfg)
		if cfg.pic ~= nil then
			m.element(cfg, "PositionIndependentCode", nil, iif(cfg.pic == "On", "true", "false"))
		end
	end
	...
	function m.androidPlatformToolset(cfg)
		...
		if _ACTION >= "vs2015" then
			...
			if cfg.toolchainversion ~= nil then
				...
				m.element(cfg, "PlatformToolset", nil, ts)
			end
		else
			local archMap = {
			...
			if (cfg.architecture ~= nil or cfg.toolchainversion ~= nil) and archMap[arch] ~= nil then
				...
				m.element(cfg, "PlatformToolset", nil, toolset .. version)
				m.element(cfg, "AndroidArch", nil, archMap[arch])
			end
		end
	end
	...
	function m.androidStlType(cfg)
		if cfg.stl ~= nil then
			...
			if _ACTION >= "vs2015" then
				m.element(cfg, "UseOfStl", nil, runtimeLib)
			else
				m.element(cfg, "AndroidStlType", nil, runtimeLib)
			end
		end
	end
	...
	function m.androidAPILevel(cfg)
		if cfg.androidapilevel ~= nil then
			m.element(cfg, "AndroidAPILevel", nil, "android-" .. cfg.androidapilevel)
		end
	end
	...
	function m.androidShortEnums(cfg)
		if cfg.flags.UseShortEnums ~= nil then
			m.element(cfg, UseShortEnums, nil, "true")
		end
	end
	...
	function m.androidLinkSections(cfg)
		...
		if cfg.linksectiondata == 'On' then
			m.element(cfg, "DataLevelLinking", nil, "true")
		elseif cfg.linksectiondata == 'Off' then
			m.element(cfg, "DataLevelLinking", nil, "false")
		end
	...
	function m.thumbMode(cfg)
		if cfg.thumbmode ~= nil then
			...
			m.element(cfg, "ThumbMode", nil, thumbMode[cfg.thumbmode])
		end
	end
	...
	function m.androidGenerateMapFile(cfg)
		if cfg.flags.Maps then
			...
			m.element(cfg, "GenerateMapFile", nil, cfg.project.name..".map")
		end
	end
	...
	function m.androidUseMultiToolTask(cfg)
		...
		if cfg.flags.MultiProcessorCompile then
			m.element(cfg, "UseMultiToolTask", nil, "true")
		end
	end
	...
	function m.androidAdditionalDependencies(cfg, explicit)
		...
		if #links > 0 then
			links = path.translate(table.concat(links, ";"))
			m.element(cfg, "LibraryDependencies", nil, "%%(LibraryDependencies);%s", links)
		end
