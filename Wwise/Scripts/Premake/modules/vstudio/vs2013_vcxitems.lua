...
--
	...
	function m.msbuildAllProjects(prj)
		vc2010.element(cfg, "MSBuildAllProjects", nil, "$(MSBuildAllProjects);$(MSBuildThisFileFullPath)")
	end

	function m.hasSharedItems(prj)
		vc2010.element(cfg, "HasSharedItems", nil, "true")
	end

	function m.itemsProjectGuid(prj)
		vc2010.element(cfg, "ItemsProjectGuid", nil, "{%s}", prj.uuid)
	end

	function m.itemsProjectName(prj)
		if prj.name ~= prj.filename then
			vc2010.element(cfg, "ItemsProjectName", nil, "%s", prj.name)
		end
	end
...
--
	...
	function m.additionalIncludeDirectories(prj)
		vc2010.element(cfg, "AdditionalIncludeDirectories", nil, '%s', '%(AdditionalIncludeDirectories);$(MSBuildThisFileDirectory)')
	end
