...
--
	...
	function m.androidAntPackage(cfg)
		...
		if cfg.androidapplibname ~= nil then
			vc2010.element(cfg, "AndroidAppLibName", nil, cfg.androidapplibname)
		else
			vc2010.element(cfg, "AndroidAppLibName", nil, "$(RootNamespace)")
		end
		vc2010.element(cfg, "AntTarget", nil, iif(premake.config.isDebugBuild(cfg), "debug", "release"))
		p.pop('</AntPackage>')
	end
