	...
	function cpp.linkCmd(cfg, toolset)
		if cfg.kind == p.STATICLIB then
			if cfg.architecture == p.UNIVERSAL and cfg.platforms["x64"] == nil then -- AUDIOKINETIC only use libtool on mac
				...
				p.outln('ifeq (posix,$(SHELLTYPE))')
				p.outln('  LINKCMD = $(AR) -rcs "$@" $(OBJECTS)')
				p.outln('else')
				p.outln('  LINKARGS = -rcs "$@" $(OBJECTS)')
				p.outln('  LINKCMD = $(AR) @linkargs.make')
				p.outln('endif')
				-- AUDIOKINETIC END
			end
		elseif cfg.kind == p.UTILITY then
			...
			p.outln('else')
			p.outln('  LINKARGS = $(OBJECTS) $(RESOURCES)')
			p.outln('  LINKCMD = $(' .. cc .. ') -o "$@" @linkargs.make $(ALL_LDFLAGS) $(LIBS) $(EXTRA_LDFLAGS)')
			p.outln('endif')
			-- AUDIOKINETIC END
		end
	end
