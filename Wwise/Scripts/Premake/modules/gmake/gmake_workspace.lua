...
--
	...
	function gmake.configmap(wks)
		...
		for cfg in p.workspace.eachconfig(wks) do
			...
			-- AUDIOKINETIC BEGIN: Support per config disable of the jobserver
			if cfg.flags.DisableMakeJobserver then
				_p('  .NOTPARALLEL:')
			end
			-- AUDIOKINETIC END

			for prj in p.workspace.eachproject_filtered(wks) do
...
--
	...
	function gmake.workspacePhonyRule(wks)
		local groups = {}
		local tr = p.workspace.grouptree_filtered(wks)
		tree.traverse(tr, {
			onbranch = function(n)
